...
// Loads a fresh module instance with a single custom rule.
// jest.isolateModules is synchronous, so fn is always assigned before return.
...
describe("rule-engine evaluate()", () => {
  let evaluate: (event: MetricEvent) => Rule[];

  beforeEach(() => {
    // Fresh module for each test — resets the lastFired Map and reloads rules.json.
    // jest.doMock() leaks out of jest.isolateModules(), so explicitly unmock
    // rules.json here to ensure real data is used for every non-isolated test.
    jest.resetModules();
    jest.restoreAllMocks();
    jest.unmock("../rules.json");
    ({ evaluate } = require("../rule-engine"));
  });

  // --- rule matching ---

  test("event triggers rule when source, metric, and threshold all match", () => {
    const triggered = evaluate(BASE_EVENT); // value 7.5 > threshold 5
    expect(triggered).toHaveLength(1);
    expect(triggered[0].name).toBe("Checkout high error rate");
  });

  test("event does not trigger rule when metric differs", () => {
    expect(evaluate({ ...BASE_EVENT, metric: "latency_ms" })).toHaveLength(0);
  });

  test("event does not trigger rule when source differs", () => {
    expect(evaluate({ ...BASE_EVENT, source: "payment-worker" })).toHaveLength(0);
  });

  test("rule with empty source matches events from any source", () => {
    const ev = loadEvalWith({ source: "" });
    // payment-worker would normally not match checkout-api, but wildcard fires it
    expect(ev({ ...BASE_EVENT, source: "payment-worker" })).toHaveLength(1);
  });

  test("disabled rule never fires", () => {
    jest.isolateModules(() => {
      jest.doMock("../rules.json", () => [{ ...DEMO_RULE, enabled: false }]);
      const { evaluate: evalDisabled } =
        require("../rule-engine") as { evaluate: typeof evaluate };
      expect(evalDisabled(BASE_EVENT)).toHaveLength(0);
    });
  });
  ...
  test("= fires when value exactly equals threshold", () => {
    // Set threshold to 7.5 to match BASE_EVENT value
    expect(loadEvalWith({ operator: "=", threshold: 7.5 })(BASE_EVENT)).toHaveLength(1);
  });

  test("= does not fire when value differs from threshold", () => {
    // BASE_EVENT value is 7.5, threshold is 5 — not equal
    expect(loadEvalWith({ operator: "=" })(BASE_EVENT)).toHaveLength(0);
  });
  ...
  test("first matching event triggers an alert", () => {
    ...
    expect(evaluate(BASE_EVENT)).toHaveLength(1);
  });

  test("second matching event within cooldown window does not fire", () => {
    jest.spyOn(Date, "now").mockReturnValue(1_000_000);
    expect(evaluate(BASE_EVENT)).toHaveLength(1); // fires; records lastFired = 1_000_000
    expect(evaluate(BASE_EVENT)).toHaveLength(0); // 0 ms elapsed — within 60 s cooldown
  });

  test("matching event after cooldown window expires fires again", () => {
    const dateSpy = jest.spyOn(Date, "now");
    dateSpy.mockReturnValue(1_000_000);
    expect(evaluate(BASE_EVENT)).toHaveLength(1); // first alert

    dateSpy.mockReturnValue(1_000_000 + 60_001); // past 60 s cooldown
    expect(evaluate(BASE_EVENT)).toHaveLength(1); // fires again
  });
});
